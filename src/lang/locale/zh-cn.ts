...
const zh_cn: Partial<LangMap> = {
  ...
  "setting.sync.auto_note": "启用笔记自动同步",
  "setting.sync.auto_note_desc": "开启后，将同步笔记仓库下的笔记、图片和各类附件。",
  "setting.sync.auto_config": "启用配置项同步",
  "setting.sync.auto_config_desc": "开启后，将同步 <b>.obsidian</b> 目录下的核心配置、插件 and 主题设置，首次开启将会使用服务端配置覆盖本地配置。",
  "setting.sync.pdf_state": "开启 PDF 状态同步",
  ...
  "setting.sync.exclude_desc": "笔记和附件同步需要排除的目录 or 文件，每行一个。\n例如：Folder1/important.md",
  ...
  "setting.sync.exclude_whitelist_desc": "即使匹配了排除目录或扩展名规则，也强制同步的笔记或附件路径，每行一个。\n例如：Folder1/important.md",
  "setting.sync.config_exclude": "配置同步排除",
  "setting.sync.config_exclude_desc": "文件路径相对于 <b>.obsidian</b> 目录，每行一个目录或文件。\n例如：plugins/obsidian-fast-note-sync/data.json",
  "setting.sync.config_exclude_placeholder": "输入配置文件或目录的相对路径，每行一个",
  "setting.sync.config_exclude_whitelist": "配置同步排除白名单",
  "setting.sync.config_exclude_whitelist_desc": "即使匹配了配置排除规则，也强制同步的配置文件路径，每行一个。\n例如：plugins/obsidian-fast-note-sync/data.json",
