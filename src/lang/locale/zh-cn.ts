...
const zh_cn: Partial<LangMap> = {
  ...
  "setting.sync.auto_note_desc": "开启后将同步笔记仓库下的笔记、图片和各类附件",
  "setting.sync.auto_config": "启用配置项同步",
  "setting.sync.auto_config_desc": "开启后将同步 .obsidian 目录下的核心配置、插件 and 主题设置，首次开启将会使用服务端配置覆盖本地配置",
  "setting.sync.pdf_state": "开启 PDF 状态同步",
  "setting.sync.pdf_state_desc": "开启后将同步 PDF 查看器的阅读状态 (本设置需要开启配置项同步)",
